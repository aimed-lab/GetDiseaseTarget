        ...
        <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
          {correlations.map((p, idx) => (
            <tr key={`${p.a.id}-${p.b.id}`} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/20 transition-colors">
              <td className="p-4 pl-8 font-bold text-blue-500 dark:text-blue-500 text-[13px]">{p.a.symbol}</td>
              <td className="p-4 font-bold text-blue-500 dark:text-blue-500 text-[13px]">{p.b.symbol}</td>
...
const KnowledgeGraph = ({ targets, selectedId, onSelect, theme }: { targets: Target[], selectedId?: string, onSelect: (t: Target | null) => void, theme: Theme }) => {
  ...
  useEffect(() => {
    ...
    targets.forEach(t => {
      ...
      ctx.fillStyle = isSel ? '#3b82f6' : (theme === 'dark' ? '#334155' : '#737373');
      ctx.fill();
  ...
  const draw = useCallback(() => {
    ...
    if (ctx) {
      ...
      mappedTargets.forEach(t => {
        ...
        ctx.shadowBlur = 0;
        if (viewport.scale > 1.2) { ctx.font = `bold ${10 / viewport.scale}px Inter`; ctx.fillStyle = theme === 'dark' ? '#f5f5f5' : '#111827'; ctx.textAlign = 'center'; ctx.fillText(t.symbol, t.x!, t.y! + (finalRadius + 8 / viewport.scale)); }
      });
      ctx.restore();
    }
    ...
    <div className={`relative w-full h-full rounded-xl overflow-hidden border flex flex-col ${theme === 'dark' ? 'bg-[#0a0a0a] border-neutral-800' : 'bg-white border-neutral-200'}`}>
      ...
      {mode === 'survival' && survivalTab !== 'default' && (
        <div className={`p-4 border-t ${theme === 'dark' ? 'bg-[#0d0d0d] border-neutral-800' : 'bg-blue-50 border-neutral-100'}`}>
          <div className="flex items-center gap-2 mb-1"><Info className="w-3.5 h-3.5 text-blue-500" /><span className="text-[11px] font-bold text-blue-500 uppercase">Independent Survival Landscape</span></div>
          <p className="text-[11px] text-neutral-500 leading-relaxed italic">{survivalTab === 'high' ? "Direct mapping of mean expression for long-surviving patients. Highlights areas of baseline molecular intensity in this cohort." : "Direct mapping of mean expression for short-surviving patients. Highlights areas of baseline molecular intensity in this cohort."}</p>
        </div>
      )}
    </div>
  );
...
const DrugLandscape = ({ targetId, theme }: { targetId: string, theme: Theme }) => {
  ...
  return (<div className="space-y-5"><div className="flex items-center gap-2"><Pill className="w-4 h-4 text-neutral-400" /><h4 className="text-[11px] font-bold uppercase text-neutral-500">Pipeline Evidence</h4></div><div className="space-y-2">{drugs.slice(0, 4).map(d => (<div key={d.id} className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-[#171717] border-neutral-800' : 'bg-neutral-50 border-neutral-200'}`}><div className="flex items-center justify-between mb-2"><span className="text-[11px] font-bold text-blue-500 uppercase">{d.name}</span><span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-neutral-200 text-neutral-600">PHASE {d.phase}</span></div><p className={`text-[11px] leading-relaxed italic ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>{d.mechanism}</p></div>))}</div></div>);
};
...
const App = () => {
  ...
  const analyzeBrcaSurvival = useCallback(async () => {
    ...
    try {
      ...
      const allRawMeans = [
        ...Object.values(metrics).map(m => m.meanHigh),
        ...Object.values(metrics).map(m => m.meanLow)
      ];
      
      const minAll = (d3 as any).min(allRawMeans) || 0;
      const maxAll = (d3 as any).max(allRawMeans) || 1;
      const range = maxAll - minAll || 1e-6;

      Object.keys(metrics).forEach(symbol => {
        const m = metrics[symbol];
        m.highDiff = (m.meanHigh - minAll) / range;
        m.lowDiff = (m.meanLow - minAll) / range;
      ...
      <main className="flex-1 flex overflow-hidden relative">
        ...
        <section className="flex-1 flex flex-col p-6 overflow-hidden">
           ...
           <div className={`flex-1 rounded-xl border overflow-hidden relative ${theme === 'dark' ? 'bg-[#121212] border-neutral-800' : 'bg-white'}`}>
              ...
              {researchState.targets.length === 0 && viewMode !== 'raw' ? (<div className="h-full flex flex-col items-center justify-center p-20 text-center opacity-30"><Search className="w-12 h-12 text-neutral-400 mb-6" /><h2 className="text-lg font-bold mb-1">Awaiting Research Focus</h2></div>) : (viewMode === 'raw' || viewMode === 'survival') && !isBrcaActive ? (<div className="h-full flex flex-col items-center justify-center p-12 text-center"><div className="p-4 rounded-full bg-blue-50 dark:bg-blue-900/10 mb-6"><AlertCircle className="w-10 h-10 text-blue-500" /></div><h3 className="text-lg font-bold mb-2">Restricted Context</h3><p className="text-sm max-w-md text-neutral-500">Analytics only available for BRCA studies in this release.</p></div>) : (
                <>
                  {viewMode === 'list' && (
                    <div className="h-full flex flex-col">
                      <div className="flex-1 overflow-auto">
                        <table className="w-full text-left border-collapse">
                          <thead className={`sticky top-0 z-10 text-[10px] font-bold uppercase tracking-widest border-b ${theme === 'dark' ? 'bg-[#171717] border-neutral-800 text-neutral-500' : 'bg-neutral-50 border-neutral-200 text-neutral-600'}`}>
                            <tr><th className="p-4 pl-8">Gene</th><th className="p-4 hidden md:table-cell">Gene Name</th><th className="p-4 text-center">Genetic</th><th className="p-4 text-center">Expression</th><th className="p-4 text-center">Target</th><th className="p-4 pr-8 text-right">Score</th></tr>
                          </thead>
                          <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                            {researchState.targets.map(t => (
                              <tr key={t.id} onClick={()=>setResearchState(p=>({...p, focusSymbol: t.symbol}))} className={`cursor-pointer transition-colors hover:bg-neutral-50/50 ${researchState.focusSymbol === t.symbol ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}>
                                <td className="p-4 pl-8 font-bold text-blue-500 dark:text-blue-500 text-[13px]">{t.symbol}</td>
                        ...
                        {researchState.activeDisease && (
                          <div className="p-8 flex flex-col items-center gap-4 border-t border-neutral-100 dark:border-neutral-800">
                            <button onClick={() => handleToolExecution('load_more', {})} disabled={loading} className={`group px-8 py-3.5 rounded-xl bg-blue-500 text-white text-[12px] font-bold uppercase tracking-widest hover:bg-blue-600 active:scale-95 transition-all flex items-center gap-3 shadow-lg shadow-blue-500/20 disabled:opacity-50 disabled:pointer-events-none`}>{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4 group-hover:rotate-90 transition-transform" />} Load More Target Evidence</button>
                  ...
                  {viewMode === 'enrichment' && (<div className="p-10 h-full overflow-auto space-y-6"><div className={`flex items-center justify-between border-b pb-4 ${theme === 'dark' ? 'border-neutral-800' : 'border-neutral-200'}`}><h4 className="text-[12px] font-bold uppercase text-neutral-500">Pathway Analytics</h4></div><div className="grid grid-cols-1 gap-4">{researchState.enrichment.map((e, i) => { const nCoCo = Math.min(0.95, (Math.log10(e.combinedScore + 1) / 3)); return (<div key={i} className={`p-5 rounded-lg border shadow-sm ${theme === 'dark' ? 'bg-[#171717] border-neutral-800' : 'bg-white border-neutral-200'}`}><div className="flex flex-col lg:flex-row justify-between gap-6"><div className="space-y-3 flex-1"><div className="flex items-center gap-3"><span className={`text-[14px] font-bold ${theme === 'dark' ? 'text-neutral-100' : 'text-neutral-900'}`}>{e.term}</span></div><div className="flex flex-wrap gap-1.5">{e.genes.slice(0, 12).map(g => (<span key={g} className={`px-2 py-0.5 rounded text-[10px] font-bold border ${theme === 'dark' ? 'bg-neutral-800 text-neutral-500 border-neutral-700' : 'bg-blue-50/50 text-blue-600 border-blue-100'}`}>{g}</span>))}</div></div><div className="flex items-center gap-10 shrink-0"><div className="text-right"><div className="text-[10px] font-bold uppercase mb-0.5 text-neutral-500">p-Value</div><div className="text-sm font-mono font-bold text-blue-500">{e.pValue.toExponential(2)}</div></div><div className="w-32 space-y-2"><div className="flex justify-between items-end"><span className="text-[10px] font-bold uppercase text-neutral-500">Magnitude</span><span className="text-[11px] font-bold font-mono">{nCoCo.toFixed(3)}</span></div><div className={`h-1.5 rounded-full overflow-hidden ${theme === 'dark' ? 'bg-neutral-800' : 'bg-neutral-100'}`}><div className="h-full bg-blue-500" style={{width: `${nCoCo*100}%`}} /></div></div></div></div></div>); })}</div></div>)}
        ...
        {researchState.focusSymbol && (<><aside className={`border-l flex flex-col gap-8 overflow-y-auto scrollbar-thin transition-all duration-300 ${isRightSidebarOpen ? 'w-[420px]' : 'w-0 opacity-0'} ${theme === 'dark' ? 'bg-[#0d0d0d] border-neutral-800' : 'bg-white shadow-sm'} p-8`}>{(() => { const t = researchState.targets.find(x => x.symbol === researchState.focusSymbol); if (!t) return null; return (<><div className="flex items-start justify-between border-b border-neutral-100 dark:border-neutral-800 pb-6"><div><h3 className="text-3xl font-extrabold text-blue-500 tracking-tight">{t.symbol}</h3><p className={`text-[12px] font-bold uppercase mt-2 ${theme === 'dark' ? 'text-neutral-500' : 'text-neutral-600'}`}>{t.name}</p></div><button onClick={() => setIsRightSidebarOpen(false)}><PanelRight className="w-4 h-4 text-neutral-400" /></button></div><div className="grid grid-cols-2 gap-3">{[ {l:'Genetic',v:t.geneticScore}, {l:'Expression',v:t.combinedExpression || 0}, {l:'Target',v:t.targetScore}, {l:'Priority',v:t.overallScore} ].map(s=>(<div key={s.l} className={`p-4 rounded-lg border shadow-sm ${theme === 'dark' ? 'bg-[#171717] border-neutral-800' : 'bg-neutral-50 border-neutral-200'}`}><div className="text-[10px] font-bold uppercase text-neutral-500 mb-1">{s.l}</div><div className="text-lg font-bold font-mono tracking-tight">{s.v.toFixed(3)}</div></div>))}</div><DrugLandscape targetId={t.id} theme={theme} /><LiteratureStats symbol={t.symbol} diseaseName={researchState.activeDisease?.name || "Evidence"} theme={theme} /></>) })()}</aside>{!isRightSidebarOpen && (<button onClick={() => setIsRightSidebarOpen(true)} className="absolute right-4 bottom-4 z-20 p-2.5 rounded-full bg-blue-500 text-white shadow-lg"><PanelRight className="w-5 h-5" /></button>)}</>)}
      </main>
    </div>
  );
};
...
const SignInPage = ({ theme, toggleTheme, onSignIn }: { theme: Theme, toggleTheme: () => void, onSignIn: (user: string) => void }) => {
  ...
  return (
    <div className={`h-screen flex items-center justify-center ${theme === 'dark' ? 'bg-[#0a0a0a]' : 'bg-neutral-50'}`}><div className={`w-full max-w-sm p-10 rounded-xl border ${theme === 'dark' ? 'bg-[#171717] border-neutral-800 shadow-2xl' : 'bg-white border-neutral-200 shadow-xl'}`}><div className="flex flex-col items-center gap-4 mb-10 text-center"><div className="p-3 bg-blue-500 rounded shadow-md"><FlaskConical className="w-8 h-8 text-white" /></div><div><h1 className={`text-2xl font-extrabold ${theme === 'light' ? 'text-neutral-900' : 'text-white'}`}>Get<span className="text-blue-500">Gene</span></h1><p className="text-[11px] font-bold uppercase tracking-widest text-neutral-400">Portal Login</p></div></div><form onSubmit={e=>{e.preventDefault(); if(password===HARDCODED_PASSWORD) onSignIn("Researcher");}} className="space-y-4"><input type="email" value={email} onChange={e=>setEmail(e.target.value)} className={`w-full p-3.5 rounded-lg border text-sm font-bold outline-none ${theme === 'dark' ? 'bg-[#0a0a0a] border-neutral-800 text-white' : 'bg-neutral-50 border-neutral-200 text-neutral-900'}`} placeholder="xxxx@uab.edu" /><input type="password" value={password} onChange={e=>setPassword(e.target.value)} className={`w-full p-3.5 rounded-lg border text-sm font-bold outline-none ${theme === 'dark' ? 'bg-[#0a0a0a] border-neutral-800 text-white' : 'bg-neutral-50 border-neutral-200 text-neutral-900'}`} placeholder="Password" /><button type="submit" className="w-full p-3.5 bg-blue-500 text-white rounded-lg font-bold uppercase text-[12px] tracking-widest shadow-md flex items-center justify-center gap-2"><ShieldCheck className="w-4 h-4" /> Authenticate</button></form><div className="mt-8 pt-8 border-t border-neutral-100 dark:border-neutral-800 flex justify-center"><button onClick={toggleTheme} className="p-2 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded transition-colors">{theme === 'dark' ? <Sun className="w-4 h-4 text-neutral-400" /> : <Moon className="w-4 h-4 text-neutral-500" />}</button></div></div></div>
  );
};
